...
export class UsersService {
        ...
        try {
            ...
            if (savedUser.status === UserStatus.ACTIVE) {
                try {
                    ...
                    this.logger.warn(`Failed to send welcome email to ${savedUser.email}`);
                }
            }
    ...
    async update(id: string, updateUserDto: UpdateUserDto): Promise<User> {
        ...
        try {
            ...
            this.logger.log(`User updated: ${updatedUser.email} (ID: ${updatedUser.id})`);
            return updatedUser;
        } catch (error) {
        ...
        try {
            ...
            if (!user) {
                ...
                user = await this.create({
                    ...
                    emailVerifiedAt: clerkUser.emailAddresses[0]?.verification?.status === 'verified'
                ...
                await this.update(user.id, {
                    ...
                    profileImageUrl: clerkUser.imageUrl,
                    emailVerifiedAt: clerkUser.emailAddresses[0]?.verification?.status === 'verified'
                        ? new Date().toISOString() : user.emailVerifiedAt?.toISOString(),
                });
    ...
    async activate(id: string): Promise<User> {
        ...
        this.logger.log(`User activated: ${updatedUser.email} (ID: ${updatedUser.id})`);
        return updatedUser;
    }
    ...
    async deactivate(id: string): Promise<User> {
        ...
        this.logger.log(`User deactivated: ${updatedUser.email} (ID: ${updatedUser.id})`);
        return updatedUser;
    }
