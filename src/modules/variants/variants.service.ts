...
export class VariantsService {
  ...
  async create(createVariantDto: CreateVariantDto): Promise<Variant> {
    ...
    if (car.brandId !== brandId) {
      throw new BadRequestException(
        'Car does not belong to the specified brand',
      );
    }
    ...
    if (existingVariant) {
      throw new ConflictException(
        'Variant with this name already exists for this car',
      );
    }
    ...
    try {
      ...
      this.logger.log(
        `Variant created: ${savedVariant.name} for car ${car.name} (ID: ${savedVariant.id})`,
      );
      return savedVariant;
    } catch (error) {
  ...
  async findAll(
    ...
    if (filterDto) {
      if (filterDto.brandId) {
        queryBuilder.andWhere('variant.brandId = :brandId', {
          brandId: filterDto.brandId,
        });
      }

      if (filterDto.carId) {
        queryBuilder.andWhere('variant.carId = :carId', {
          carId: filterDto.carId,
        });
      }
  ...
  async update(
    ...
    if (updateVariantDto.name && updateVariantDto.name !== variant.name) {
      const existingVariant = await this.findByCarAndName(
        variant.carId,
        updateVariantDto.name,
      );
    ...
    try {
      ...
      Object.assign(variant, {
        ...
        description:
          updateVariantDto.description?.trim() || variant.description,
        engineCapacity:
          updateVariantDto.engineCapacity?.trim() || variant.engineCapacity,
      ...
      this.logger.log(
        `Variant updated: ${updatedVariant.name} (ID: ${updatedVariant.id})`,
      );
  ...
  async updateSortOrder(
    ...
    try {
      ...
      this.logger.error(
        `Failed to update variant sort order: ${error.message}`,
      );
  ...
  async updateStock(id: string, stockQuantity: number): Promise<Variant> {
    ...
    this.logger.log(
      `Variant stock updated: ${updatedVariant.name} - Stock: ${updatedVariant.stockQuantity}`,
    );
    return updatedVariant;
  }
  ...
  async activate(id: string): Promise<Variant> {
    ...
    this.logger.log(
      `Variant activated: ${updatedVariant.name} (ID: ${updatedVariant.id})`,
    );
    return updatedVariant;
  }
  ...
  async deactivate(id: string): Promise<Variant> {
    ...
    this.logger.log(
      `Variant deactivated: ${updatedVariant.name} (ID: ${updatedVariant.id})`,
    );
    return updatedVariant;
  }
  ...
  async discontinue(id: string): Promise<Variant> {
    ...
    this.logger.log(
      `Variant discontinued: ${updatedVariant.name} (ID: ${updatedVariant.id})`,
    );
    return updatedVariant;
  }
  ...
  async getStats(): Promise<{
    ...
    const [
      ...
      this.variantsRepository.count({
        where: { status: VariantStatus.INACTIVE },
      }),
      this.variantsRepository.count({
        where: { status: VariantStatus.DISCONTINUED },
      }),
      this.variantsRepository.count({
        where: { status: VariantStatus.OUT_OF_STOCK },
      }),
