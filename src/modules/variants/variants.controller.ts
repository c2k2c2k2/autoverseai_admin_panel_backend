...
export class VariantsController {
    ...
    async findAll(
        @Query() filterDto: FilterVariantsDto,
    ) {
        const result = await this.variantsService.findAll(filterDto, filterDto);

        return {
            ...result,
            data: result.data.map(variant => plainToInstance(VariantResponseDto, variant)),
        };
    }
    ...
    async findAllActive(): Promise<VariantResponseDto[]> {
        ...
        return variants.map(variant => plainToInstance(VariantResponseDto, variant));
    }
    ...
    async updateStock(
        ...
        const variant = await this.variantsService.updateStock(id, updateStockDto.stockQuantity);
        return plainToInstance(VariantResponseDto, variant);
    }
